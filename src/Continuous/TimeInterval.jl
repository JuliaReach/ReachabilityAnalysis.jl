...
min(Δt::TimeInterval{<:Interval}) = low(Δt.i, 1)
min(Δt::TimeInterval{<:IA.Interval}) = inf(Δt.i)

max(Δt::TimeInterval{<:Interval}) = high(Δt.i, 1)
max(Δt::TimeInterval{<:IA.Interval}) = sup(Δt.i)

tstart(Δt::TimeInterval) = min(Δt)
tend(Δt::TimeInterval) = max(Δt)
...
function _isapprox(Δt::TimeInterval, Δs::TimeInterval)
    return (min(Δt) ≈ min(Δs)) && (max(Δt) ≈ max(Δs))
end

@commutative function +(Δt::TimeInterval{<:Interval}, t::Real)
    return TimeInterval(Interval(min(Δt) + t, max(Δt) + t))
end
...
function isdisjoint(Δt::TimeInterval, Δs::TimeInterval)
    return min(Δt) > max(Δs) || min(Δs) > max(Δt)
end
...
@commutative function _intersection(x::IA.Interval, y::Interval)
    return _intersection_interval(x, y.dat)
end

function _intersection(x::IA.Interval, y::IA.Interval)
    l, h = max(inf(x), inf(y)), min(sup(x), sup(y))
    if l > h
        throw(ArgumentError("the intervals $x and $y do not intersect"))
    end
    return interval(l, h)
end
