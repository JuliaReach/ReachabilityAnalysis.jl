function post(alg::TMJets20{N}, ivp::IVP{<:AbstractContinuousSystem}, timespan;
    ...
    if external
        return _solve_external(f!, X0, t0, T, orderQ, orderT, abstol, maxsteps, Δt0;
                               parse_eqs=parse_eqs, kwargs...)
    end
    ...
    F, tv, xv, xTM1v, success, _t0 = validated_integ!(F, f!, X0tm, t0, T, orderQ, orderT,
                                                      abstol, maxsteps, X, disjointness, Δt0,
                                                      adaptive; parse_eqs=parse_eqs)
    ...
    ext = Dict{Symbol,Any}(:tv => tv_vec, :xv => xv_vec, :xTM1v => xTM1v_vec,
                           :actual_abs_tol => abstol_vec)
    return Flowpipe(F, ext)
end
