"""
    TMJets26{N, DM<:AbstractDisjointnessMethod} <: AbstractContinuousPost
...
"""
@with_kw struct TMJets26{N,DM<:AbstractDisjointnessMethod} <: AbstractContinuousPost
...
numtype(::TMJets26{N}) where {N} = N
rsetrep(::TMJets26{N}) where {N} = TaylorModelReachSet{N}
