...
@testset "QINT algorithm" begin
    ...
    @test_broken RA.QINTModule.reach_homog_QINT(alg, ivp, tspan) isa HybridFlowpipe  # TODO finish implementation; method should also be called `post` so one can call `solve`

    # call `reach_homog_QINT` directly
    # x' = x - x^2, with solution x(t) = x0*e^t / (1 - (1 - e^t)x0)
    sol = RA.QINTModule.reach_homog_QINT(; a=-1.0, b=1.0, c=0.0, X0=Interval(0.1, 0.3),
