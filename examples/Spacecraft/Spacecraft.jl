...
function velocity_constraint(sol)
    ...
    for idx in all_idx
        ...
        verif = all(absolute_velocity(R) < 0.055 * 60.0 for R in sol[idx])
        !verif && return false
    end
    return true
end
...
plot!(fig, solz[idx_approaching[1]]; vars=(1, 2), lw=0.0, color=:lightgreen, alpha=1.0,
      lab="Approaching")
for k in 2:length(idx_approaching)
    plot!(fig, solz[idx_approaching[k]]; vars=(1, 2), lw=0.0, color=:lightgreen, alpha=1.0)
end

plot!(fig, solz[idx_attempt[1]]; vars=(1, 2), lw=0.0, color=:red, alpha=1.0, lab="Attempt")
for k in 2:length(idx_attempt)
    plot!(fig, solz[idx_attempt[k]]; vars=(1, 2), lw=0.0, color=:red, alpha=1.0)
end

plot!(fig, solz[idx_aborting[1]]; vars=(1, 2), lw=0.0, color=:cyan, alpha=1.0, lab="Aborting")
for k in 2:length(idx_aborting)
    plot!(fig, solz[idx_aborting[k]]; vars=(1, 2), lw=0.0, color=:cyan, alpha=1.0)
end
