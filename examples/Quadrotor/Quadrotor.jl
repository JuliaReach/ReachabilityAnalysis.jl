...
# We study the dynamics of a quadrotor, derived in [Beard08](@citet) and used in
# [GerettiSABCCDFK21](@citet).
...
# quadrotor according to [Beard08; Eq. (16)-(19)](@citet):
#
# ```math
# \begin{aligned}
...
@inline function quad_property(sol)
    ...
    b1 = all([isdisjoint(unsafe1, set(R)) for R in sol(interval(0.0, tf))])
    #b1 = ρ(v3, sol) < 1.4

    ## Condition: x[3] > 0.9 for t ≥ 1.0
    unsafe2 = HalfSpace(v3, 0.9) # unsafe: x3 <= 0.9
    b2 = all([isdisjoint(unsafe2, set(R)) for R in sol(interval(1.0, tf))])
